...
memalloc_start(PyObject* Py_UNUSED(module), PyObject* args)
{
    ...
    if (heap_sample_size < 1 || heap_sample_size > MAX_HEAP_SAMPLE_SIZE) {
        PyErr_Format(PyExc_ValueError, "the heap sample size must be in range [1; %lu]", MAX_HEAP_SAMPLE_SIZE);
        return NULL;
    }
